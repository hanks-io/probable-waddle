<template>
  <div class="button flex items-end" :class="{shiny: shiny && !disabled && !suffixLoading && !prefixLoading}" :style="{height: buttonHeight, width: width || '100%'}">
    <ion-button class="amber-button" mode="md" @click="handleClick" :class="{disabled: disabled, loading: suffixLoading || prefixLoading}" :style="{'--background': background}" v-bind="$attrs" @click.stop>
...
const ellipsis = ref('')
const buttonHeight = props.height || '2.875rem';
...
div.button ion-button.amber-button {
...
div.button ion-button.amber-button.ion-activated {
  height: calc(v-bind(buttonHeight) - 2.4px)
}

div.button ion-button.amber-button.ion-activated::part(native) {
  box-shadow: none;
}

div.button ion-button.amber-button p {
  font-size: inherit;
}

div.button ion-button.amber-button p span {
  color: var(--button-text-color, --text-color-white-100); /* 默认白色文字 */
}

div.button ion-button.amber-button.disabled p span {
  color: var(--color-primary-btn-text-disable); /* 默认白色文字 */
}

:global(ion-button.amber-button p) {
...
ion-button.disabled {
  opacity: 0.4;
  --background: var(--color-disable) !important; /* 禁用状态背景色 */
  --color: var(--text-color-white-60); /* 禁用状态文字颜色 */
}

ion-button.loading {
  --background: var(--color-load) !important; /* 加载状态背景色 */
  --color: rgba(45, 45, 45, 0.6);; /* 加载状态文字颜色 */
}
