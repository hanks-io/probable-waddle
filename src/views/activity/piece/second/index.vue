  ...
  <ion-page>
    ...
    <ion-content>
      ...
      <div class="px-10 pt-6">
        ...
        <div class="text-[.875rem] text-center my-2 line-clamp-2 color-text-100">
      ...
      <div class="h-[23.875rem] w-[24.375rem] relative" :style="wheelBoxStyle">
        ...
        <div class="relative flex justify-center mr-1">
          ...
          <div class="absolute top-0 bottom-0 left-0 right-0 z-20 flex items-center justify-center w-48 h-48 m-auto rounded-full wheel-start">
            <div class="button text-white w-16 h-[5.31rem] flex items-center justify-center" :class="{ on: startClick }">
          ...
          <div class="scroll-box text-xs record-text leading-loose"
            ...
            <ion-row v-for="item in assistanceCashAwards" :key="item.userId">
              ...
              <ion-row class="color-text-currency items-center">
                ...
                <p v-if="tenantInfo?.merchantCy" class="ml-2 h-4 w-4 text-center bg-[#6ddf39] leading-[1.125rem] text-[.625rem] color-text-80 rounded-full font-black">
          ...
          <div v-show="recordTabs" class="overflow-y-auto h-40 hide-scrollbar">
            <div class="record-text text-xs leading-loose min-h-full bg-200">
              <ion-row v-for="item in assistanceCashHelps" :key="item.userId">
                ...
                <ion-row class="color-text-currency items-center">
                  + {{ convertMoneyToShow(item.helpAmount ?? 0) }} <p v-if="tenantInfo?.merchantCy"
                    class="ml-2 h-4 w-4 text-center bg-[#6ddf39] leading-[1.125rem] text-[.625rem] color-text-80 rounded-full font-black">
                    {{ tenantInfo?.merchantCy }}</p>
                </ion-row>
              </ion-row>
            </div>
          </div>
      ...
      <!-- 规则 -->
      <p class="rounded-middle mb-[1.25rem] mx-5 p-[0.625rem] bg-200 text-xs rule-text keep-space">{{ rule }}</p>
    </ion-content>
    <!-- 帮助弹窗 -->
    ...
    <ion-modal id="modal-share" :is-open="modalShareVisible" :initial-breakpoint="1" :backdrop-dismiss="false">
      <div class="flex flex-col justify-end h-full">
        <div class="p-3 color-text-50 bg-300 relative">
          <!-- 关闭按钮 -->
