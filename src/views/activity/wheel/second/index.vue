			...
			<ion-content>
				<div class="wheel w-full flex flex-col justify-end h-[36.5625rem]">
					<div class="h-full flex flex-col justify-between relative">
						...
						<div class="flex-1 flex items-center justify-center text-base">
							<div v-if="calcStatus">
								<span class="py-1 px-1.5 rounded-middle color-text-40 bg-200">
									...
									<span
										v-if="activityStatus">{{ showEndCountdown ? $t('activity.end') : $t('activity.foreverEnd') }}</span>
						...
						<!-- 转盘台 -->
						<div class="wheel-table w-[22.6875rem] h-[28.65625rem] flex justify-center pt-[3.4rem] mx-auto relative">
							<!-- 光圈 -->
							...
							<div class="relative h-[15.75rem]">
								<LuckyWheel ref="luckyRef" :blocks="blocks" width="15.75rem" height="15.75rem" :prizes="prizes"
									:buttons="buttons"
									:defaultConfig="{ gutter: 2, speed: 25, accelerationTime: 1500, decelerationTime: 3500 }"
									@start="startCallback" @end="endCallback" />
								<!-- 转盘开始按钮 -->

								<img class="wheel-start z-20" :class="{ on: startClick }" src="/svg/activity/wheel_start.svg" />
								<!-- 动画元素		 -->
								...
								<img v-show="isShowAnimationEl"
									:class="['finger', { 'fingerAnimation': wheelInfo.lotteryTicketGetCount }]"
				...
				<div class="flex justify-around px-1">
					<div class="w-[4.375rem] h-[5.625rem] flex flex-col rounded-middle" :class="isRedeemable ? 'shiny' : ''"
						:style="setCardStyle(`prop_${item.name}`)" v-for="(item, index) in rewardCards" :key="index">
				...
				<div class="flex flex-col items-center mt-4">
					...
					<div class="flex items-center mt-4">
						...
						<span
							class="font-weight-bold text-lg mx-2 color-text-100">{{ $t('viewsActivity.obtainRewardTickets') }}</span>
						<img class="w-7" src="/svg/activity/adorn.svg" style="transform:scaleX(-1)" />
					</div>
					<!-- 奖券领取信息 -->
					<div class="w-[21.875rem] rounded-large bg-200 px-2.5 py-2 mb-2.5" v-for="item in wheelInfo.lotteryTicketGet"
						:key="item.uuid">
						<div class="flex justify-between">
							...
							<span
								class="color-text-80">{{ item.receiveCount > item.triggerCount ? item.triggerCount : item.receiveCount }}/{{
						...
						<div class="flex justify-between mt-1.5 ">
							...
							<div class="button h-[1.3125rem] flex items-center justify-center rounded-middle px-2" v-else
